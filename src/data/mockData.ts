...
export interface UtilityDetail {
  ...
  payments: { month: string; paid: boolean }[];
  lastBillDate: string;
}
...
export interface Property {
  ...
  rentHistory: { month: string; year: number; amount: number; paid: boolean }[];
  investment: InvestmentData;
  occupancy: OccupancyEntry[];
  marketData: MarketData;
  maintenanceItems?: MaintenanceItem[];
  periodicalChecks?: PeriodicalCheck[];
  managementAgreement: { startDate: string; endDate: string };
}
...
export function generateProperties(count: number = 100): Property[] {
  return Array.from({ length: count }, (_, i) => {
    ...
    // Some properties have a future tenancy (about to sign)
    ...
    // Utilities: transferred means transferred to new tenant's name. Vacant = always not-transferred.
    ...
    for (let yr = 2024; yr <= 2026; yr++) {
      for (let m = 0; m < 12; m++) {
        ...
        rentHistory.push({
          ...
          year: yr,
          amount: currentRent,
          paid: isPast ? (isLet ? Math.random() > 0.1 : false) : false,
        });
      }
    }
    ...
    const maintenanceDescriptions = [
      "Changed leaky pipe in bathroom",
      "Leak from the roof — waterproofing repair",
      "Broken window lock replacement",
      "Boiler servicing and repair",
      "Electrical socket replacement in kitchen",
      "Repainted living room walls",
      "Fixed broken door handle",
      "AC unit maintenance",
    ];
    const maintenanceItems: MaintenanceItem[] = Array.from({ length: randomInt(0, 4) }, () => {
      const reportDate = randomDate(2024, 2025);
      const resolved = Math.random() > 0.2;
      const resDays = randomInt(1, 30);
      return {
        description: randomFrom(maintenanceDescriptions),
        reportDate,
        resolveDate: resolved ? randomDate(2025, 2026) : undefined,
    ...
    return {
      ...
      deposit: {
        ...
        heldAt: randomFrom(["TDS", "DPS", "MyDeposits"]),
        schemeRef: `TDP-${randomInt(100000, 999999)}`,
        returnDetails: depositReturnDetails,
      },
      keys: keysConfig,
      utilities: { electricity: elecTransfer, water: waterTransfer },
      utilityDetails: [
        {
          type: "electricity",
      ...
      serviceCharges: {
        ...
        payments: scStatus === "n/a" ? [] : generatePayments(),
      },
      currentTenancy,
      futureTenancy,
      previousTenancies,
      ...
      periodicalChecks,
      managementAgreement: (() => {
        // Management agreement starts around the same time as the first tenancy
